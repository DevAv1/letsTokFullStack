...
export const useProductsFilterPicklist = (productsFromStore?: Product[]) => {
  ...
  const [productNameFilterValue, setProductNameFilterValue] = useState<string>();
  ...
  const productsPicklists = useMemo(() => {
    if (productsFromStore) {
      const productBrands = formattingPicklistDistinct(productsFromStore.map((product: Product) => product.brand))

      const productNames = formattingPicklistDistinct(productsFromStore.map((product: Product) => product.name))
      return {
        productBrands,
        productNames
      }
  ...
  const handleBrandFilter = (brandName: string) => {
    const filteredResult = productsFromStore?.filter((p: Product) => p.brand === brandName);
    setFilteredProduct(filteredResult || [])
  }

  const handleProductNameFilter = (productName: string) => {
    const filteredResult = productsFromStore?.filter((p: Product) => p.name === productName);
    setFilteredProduct(filteredResult || [])
  }
